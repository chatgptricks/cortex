...
Render application logs confirmed both `DuplicateColumn` and a second blocker exposed after fixing it: `IndeterminateDatatype: could not determine data type of parameter $13`. Optional-field preservation checks now bind integer presence flags instead of untyped `? IS NULL` expressions. The regression connection rejects both unsupported patterns, rather than relying only on SQLite's permissive typing.
