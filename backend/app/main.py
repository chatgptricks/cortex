...
@app.post("/api/admin/_temp-catchup-sentient-hot-alerts")
def temp_catchup_sentient_hot_alerts(
    password: Annotated[str, Form()], dry_run: bool = True
) -> dict[str, Any]:
    """One-time catch-up. Several Sentient-group posts were marked HOT while
    SLACK_ALERT_GROUPS still defaulted to 'competitors', so the one-shot
    alert-queueing in apify_sync (gated on hot_checked flipping to 1) never
    fired for them and never will retroactively. Now that SLACK_ALERT_GROUPS
    covers all groups, this resends those specific alerts once. Remove after
    use -- this is not meant to become a permanent endpoint.
    """
    _require_admin(password)
    from .slack_alerts import cover_url_for, notify_hot_post, slack_configured

    sentient_dashboard_accounts = ["traselveloreal", "costarica", "artificialmente.ia"]

    with connect() as conn:
        accounts_cfg = {
            row["handle"]: dict(row)
            for row in conn.execute("SELECT * FROM accounts WHERE group_name = 'sentient'").fetchall()
        }

        rows: list[dict[str, Any]] = []

        for r in conn.execute(
            "SELECT id, shortcode, caption, likes, published_at, hot_marked_at, hot_rate_multiplier "
            "FROM posts WHERE is_hot = 1"
        ).fetchall():
            d = dict(r)
            d["account"] = "chatgptricks"
            d["permalink"] = f"https://www.instagram.com/p/{d['shortcode']}/" if d.get("shortcode") else None
            rows.append(d)

        placeholders = ",".join("?" for _ in sentient_dashboard_accounts)
        for r in conn.execute(
            "SELECT id, account, shortcode, permalink, caption, likes, published_at, "
            "hot_marked_at, hot_rate_multiplier FROM dashboard_posts "
            f"WHERE is_hot = 1 AND account IN ({placeholders})",
            sentient_dashboard_accounts,
        ).fetchall():
            d = dict(r)
            if not d.get("permalink") and d.get("shortcode"):
                d["permalink"] = f"https://www.instagram.com/p/{d['shortcode']}/"
            rows.append(d)
    ...
    for row in rows:
        account = row["account"]
        cfg = accounts_cfg.get(account)
        threshold = cfg["hot_threshold"] if cfg else None
        multiplier = row.get("hot_rate_multiplier")
        rate_per_hour = (
            round(multiplier * threshold) if isinstance(multiplier, (int, float)) and threshold else None
        )

        published = parse_ts(row.get("published_at"))
        marked = parse_ts(row.get("hot_marked_at"))
        age_hours = round((marked - published).total_seconds() / 3600, 1) if published and marked else None
        ...
        alerts.append(
            {
                ...
                "cover_url": cover_url_for(account, row["id"]),
                "hot_marked_at": row.get("hot_marked_at"),
            }
        )

    if dry_run:
        return {"dry_run": True, "count": len(alerts), "alerts": alerts}
    ...
    return {"dry_run": False, "count": len(alerts), "sent": sent}
