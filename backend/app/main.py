...
def admin_diag(password: str) -> dict[str, Any]:
    """TEMPORARY -- every DB-backed route started returning 500 while /api/health
    stayed green, which means the app is up and the database is not. Surfaces the
    actual exception plus disk usage, since a full persistent disk makes SQLite
    fail on write-ahead-log operations even for plain reads. Remove after use."""
    import traceback
    ...
    try:
        ...
        out["disk"] = {"error": repr(exc)}

    try:
        p = Path(DB_PATH)
        out["db_file"] = {"path": str(p), "exists": p.exists(),
                          "size_mb": round(p.stat().st_size / 1e6, 2) if p.exists() else None}
        for suffix in ("-wal", "-shm"):
            side = Path(str(p) + suffix)
            if side.exists():
                out["db_file"][suffix] = round(side.stat().st_size / 1e6, 2)
    except Exception as exc:
        out["db_file"] = {"error": repr(exc)}

    try:
        with connect() as conn:
            out["query"] = {
                "ok": True,
                "posts": conn.execute("SELECT COUNT(*) c FROM dashboard_posts").fetchone()["c"],
            }
    except Exception as exc:
