    ...
    with connect() as conn:
        ...
        conn.executemany(
            """INSERT INTO queue_tickets
               (ticket_type, requester_email, status, block_category, scheduled_date,
                scheduled_start_minutes, duration_minutes, created_at, updated_at)
               VALUES (?, 'pd@example.com', ?, ?, ?, ?, ?, 'now', 'now')""",
            [
                ("time_block", "pending", "account_request", None, None, None),
                ("time_block", "approved", "post_suggestion", None, None, None),
                ("time_block", "pending", "move", "2026-09-01", 600, 0),
                ("time_block", "approved", "meeting", "2026-09-01", 720, 30),
            ],
        )

    with connect() as conn:
        occupied = main._queue_v2_time_occupied(conn, "pd@example.com", "2026-09-01")
